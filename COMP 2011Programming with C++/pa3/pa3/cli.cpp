...
Dir *execute(Dir *wd, char *rest, bool &exit)
{
    ...
    */
    ...
    else if (strcmp(cmd, "rm") == 0)
    {
         ...
         * This block should not print anything to the standard output.
        */
        char rmname[MAX_CMD_LEN] = "";  
        fetch(rest,rmname);
        char ifrecur[MAX_CMD_LEN] = "";
        fetch(rest,ifrecur);
        if(strcmp(rmname,"-r")==0){
            if(deleteDir(wd,ifrecur,true) != 0){
                deleteFile(wd,ifrecur);
            }
        }
        else{
            deleteFile(wd,rmname);
            deleteDir(wd,rmname,false);
        }
    ...
    else if (strcmp(cmd, "mv") == 0)
    {
         ...
         * This block should not print anything to the standard output.
        */
        char firstname[MAX_CMD_LEN] = "";
        fetch(rest,firstname);
        
        char lastname[MAX_CMD_LEN] = "";
        fetch(rest,lastname);
        bool file = false;
        bool dir = false;
        File* cur_file = wd->subfile;
        while(cur_file){
            if(strcmp(cur_file->name, firstname) == 0){
                file = true;
                break;
            }
        ...
        check_d = wd->subdir;
        while(check_d){
            if(strcmp(check_d->name, firstname) == 0){
                dir = true;
                break;
            }
        ...
        Dir* cur_dir = wd->subdir;
        while(cur_dir){
            if(strcmp(cur_dir->name, lastname) == 0){
                break;
            }
        ...
        if(file == true){
            cout << "move_file_no." << moveFile(cur_file,cur_dir);
        }
        else{
            moveDir(check_d,cur_dir);
        }
